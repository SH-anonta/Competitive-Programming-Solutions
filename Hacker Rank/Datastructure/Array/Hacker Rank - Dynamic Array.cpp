//#Name: Anonta Haque #Problm:  Hacker rank Dynamic Array
...
void solve(void)
{
    int n, q, mode, size, lastans= 0;
    LL x, y, temp;
 
    read(n);
    read(q);
    
    vector<LL>* sqn= new vector<LL>[n];
    
    for(int i= 0; i<q; i++)
    {
        sf("%d%lld%lld", &mode, &x, &y);
        
        if(mode == 1)
        {
            sqn[(x^lastans)%n].push_back(y);
        }
        else
        {
            size= sqn[(x^lastans)%n].size();
            temp= sqn[(x^lastans)%n][y% size];
            pf("%lld\n", temp);

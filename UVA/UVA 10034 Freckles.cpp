...
int ini(int n){for(int i= 0; i<=n; i++) p[i]= i;}
int rep(int a){if(p[a] == a) return a; return p[a]= rep(p[a]);}
int join(int a, int b){p[rep(a)]= rep(b);}
...
public:
    ...
    double dist(point a, point b){
        double aa = a.first-b.first, bb= a.second-b.second;
    ...
    bool operator<(edge& a){return d < a.d;}
};
...
double mst(int n){
    ...
    for(int i= 0; i<m; i++){
        if(common(id[e[i].a], id[e[i].b])) continue;
        cost+= e[i].d;
        
        join(id[e[i].a], id[e[i].b]);
    }

...
void solve(void)
{
    ...
    double r, linerDis, angle, arcDis;
...
//        cin>>Ox>>Oy>>Ax>>Ay;
        double OAx = abs(Ox - Ax);
        double OAy = abs(Oy - Ay);

        r= sqrt(OAx*OAx + OAy*OAy);

        linerDis = sqrt( abs((Ax-Bx)*(Ax-Bx)) + abs((Ay-By)*(Ay-By)) );

        angle= 2*asin(linerDis/(2*r));

        arcDis= angle * r;

        pf("Case %d: %.8f", tst, arcDis);
    }

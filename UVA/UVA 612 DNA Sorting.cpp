...
bool comp(const pair<string, int>& a,const pair<string, int>& b){return a.second<b.second;}

int sortedness(const string& s){
    int len= s.size(), u= 0;
    for(int i= 0; i<len; i++){
...
void solve(void){
    ...
    for(int Case= 1; Case<=Tc; Case++)
    {
        ...
        for(int i=0; i<m;i++){
            cin>>dna[i].first;
            dna[i].second= sortedness(dna[i].first);
        }

...
//test row
bool rowtest(string g[]){
    for(int i= 0; i<4; i++){
        if(g[i].find(string(".xx")) != string::npos 
            || g[i].find(string("xx.")) != string::npos
            || g[i].find(string("x.x")) != string::npos)
            return true;
    }
...
bool ditest(string g[]){
    int sum, tmp, mat= 'x'+'x'+'.';
    for(int i= 1; i<3; i++){
        
        for(int j= 1; j<3; j++){
            sum= 0;
            tmp =(g[i][j]+g[i-1][j+1]+g[i+1][j-1]);
            if(tmp == mat) return true;
            tmp =(g[i][j]+g[i+1][j+1]+g[i-1][j-1]);
            if(tmp == mat) return true;
        }
    }

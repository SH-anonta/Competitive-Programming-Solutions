...
int combo(int n, int p, int diff, int mx, int mn){
    int sum= 0;
    if(n == -1){
        if(p >1 &&  diff >= l && diff <= r && abs(mx - mn) >= x) return 1;
        else return 0;
    }
    
    sum += combo(n-1, p, diff, mx, mn);
    mx= max(mx, a[n]);
    mn= min(mn, a[n]);
    diff+= a[n];
    sum+= combo(n-1, p+1, diff, mx, mn);
...
void solve(void){
    ...
    cout<< combo(N-1, 0, 0, 0, INT_MAX) <<endl;
